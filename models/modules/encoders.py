...
class Encoder(nn.Module):
    def __init__(self, config):
        ...
        self.layers = nn.ModuleList(
            [EncoderLayer(config.SELF_ATTENTION) for _ in range(config.LAYERS)]
        )
...
class GeometricEncoder(nn.Module):
    ...
    def forward(
        self, features: torch.Tensor, boxes: torch.Tensor, padding_mask: torch.Tensor
    ):
        # embedding geometric information from boxes' coordinates
        relative_geometry_embeddings, delta_x, delta_y, delta_w, delta_h = (
            box_relational_embedding(
                boxes,
                dim_g=self.d_g,
        ...
        relative_geometry_weights_per_head = [
            fc_g(flatten_relative_geometry_embeddings).view(box_size_per_head)
            for fc_g in self.fc_gs
        ]
        relative_geometry_weights = torch.cat(
            relative_geometry_weights_per_head, dim=1
        )  # (bs, h, nk, nk)
...
class DualCollaborativeLevelEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.region2grid = nn.ModuleList(
            [EncoderLayer(config.CROSS_ATTENTION) for _ in range(config.LAYERS)]
        )

        # Cross Attention between grids and regions
        self.grid2region = nn.ModuleList(
            [EncoderLayer(config.CROSS_ATTENTION) for _ in range(config.LAYERS)]
        )
    ...
    def forward(
        ...
        region_padding_mask: torch.Tensor,
        region2all_mask: torch.Tensor,
        grid_features: torch.Tensor,
        grid_boxes: torch.Tensor,
        grid_padding_mask: torch.Tensor,
        grid2all_mask: torch.Tensor,
    ):
        n_regions = region_features.shape[1]

        boxes = torch.cat(
            [region_boxes, grid_boxes], dim=1
        )  # (bs, n_regions + n_grids, 4)
        relative_geometry_embeddings = box_relational_embedding(
            boxes, dim_g=self.d_g, trignometric_embedding=self.trignometric_embedding
        )
        flatten_relative_geometry_embeddings = relative_geometry_embeddings.view(
            -1, self.d_g
        )
        ...
        region_features = self.layer_norm_region(region_features) + self.pos_embedding(
            region_features
        )
        grid_features = self.layer_norm_grid(grid_features) + self.pos_embedding(
            grid_features
        )
        for l_region, l_grid, l_r2g, l_g2r in zip(
            self.layers_region, self.layers_grid, self.region2grid, self.grid2region
        ):
            # self-attention on region feature
            region_features = l_region(
                queries=region_features,
            ...
            grid_features = l_grid(
                queries=grid_features,
...
class CrossAttentionMultiLevelEncoder(nn.Module):
    ...
    def forward(self, features: torch.Tensor, padding_mask: torch.Tensor):
        ...
        out1, out2, out3 = (
            outs  # conventionally assump that we have 3 layers for encoder block
        )
