...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, queries, keys, values, attention_mask=None, delta_x=None, delta_y=None, delta_w=None, delta_h=None, **kwargs):
        b_s, nq = queries.shape[:2]
        nk = keys.shape[1]
        # print(values.shape)
        delta_att = None
        if delta_x is not None and delta_y is not None and delta_w is not None and delta_h is not None:
            # (b_s, nq, nk, 1) for each delta, concat to (b_s, nq, nk, 4)
            deltas = torch.cat([delta_x, delta_y, delta_w, delta_h], dim=-1)  # (b_s, nq, nk, 4)    
            deltas = deltas[:, :nq, :nk, :]  # Ensure shape matches queries/keys
            delta_att = self.deltas_mlp(deltas)  # (b_s, nq, nk, h)
            delta_att = delta_att.permute(0, 3, 1, 2)  # (b_s, h, nq, nk)
        ...
        if delta_att is not None:
            # print(torch.matmul(q, k).shape, delta_att.shape)
            att = (torch.matmul(q, k) + delta_att) / np.sqrt(self.d_k)  # (b_s, h, nq, nk)
        else:
