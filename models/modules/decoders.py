...
class IntegratedDecoder(Module):
    ...
    def forward(self, caption_tokens, features, boxes, padding_mask):
        ...
        # enhancing positional features
        # deltas = torch.cat([delta_x, delta_y, delta_w, delta_h], dim=-1)  # (bs, nk, nk, 4)
        # deltas = deltas[:, :nk, :nk, :]  # Ensure shape matches if needed
        # deltas_mlp_out = self.mlp(deltas)  # (bs, nk, nk, d_model)
        # deltas_diag = torch.diagonal(deltas_mlp_out, dim1=1, dim2=2).permute(2, 0, 1)  # (nk, bs, d_model) -> (bs, nk, d_model)
        
        out_feat = self.layer_norm(features) + self.pos_embedding(features)
        # out_feat = out_feat + deltas_diag
